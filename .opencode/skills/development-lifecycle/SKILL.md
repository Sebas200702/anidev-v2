...
- **7 · ARCHIVE / RELEASE.** Merge via PR; then `openspec-archive-change` to sync deltas into `openspec/specs/`. Releases follow SemVer by Conventional Commits: on `master`, `bun run release:*` bumps `package.json`, rewrites `CHANGELOG.md`, and tags `vX.Y.Z`; pushing that tag triggers CI to build+deploy the Docker image tagged `:<version>` (see AGENTS.md "Releases & Versioning").
